...
function errorCatcher(){
	console.log('\033[31mThere was an error connecting to SendGrid. Please try your request again.\033[39m\n');
}
...
function perform(action, hostname, endpoint, spam){
	program.prompt('Username: ', function(inputName){
		...
		program.password('Password: ', ' ', function(inputPassword){
			...
			// This is a massive hack, that deletes the record, then replaces it with updated information
			// because the update endpoint doesn't work... :(
			if (action === 'parse.update'){
				loginAndRequest('parse.delete', {hostname: hostname}, function(error, body){
					if (!body) {
						errorCatcher();
					} else {
						...
						if (res.error){
							...
							loginAndRequest('parse.set', {hostname: hostname, endpoint: endpoint, spam: spam}, function(error, body){
								if (!body) {
									errorCatcher();
								} else {
									var res = JSON.parse(body);
...
program
	.version('0.0.1')
	.option('-p, --parse <option>', '[add|update|delete|list] a new Parse API setting')
